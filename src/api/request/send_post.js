...
export const share = (status, okCallback, errorCallback) => {
    // 利用分享接口发送微博    
    // 向我的服务器发送Get请求
    let url = HOST_CONFIG.sendHost + "method=share" + "&text=" + encodeURI(status.text)  + " http://sureinternet.applinzi.com"
    axios.get(url)
...
const comments_reply = (status, okCallback, errorCallback) => {
    // 回复一条评论
    let url = HOST_CONFIG.sendHost + "method=comments_reply" + "&text=" + status.text + "&id=" + status.id + "&cid" + status.cid
...
export const comments_destroy = (status, okCallback, errorCallback) => {
    // 删除一条我的评论
    let url = HOST_CONFIG.sendHost + "method=comments_destroy" + "&cid=" + status.cid
    axios.get(url)
        .then(function (response) {
            okCallback(response)
        })
